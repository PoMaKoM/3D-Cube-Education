  ...
  if (name) {
    ...
    if (events) {
      if (fn) {
        ...
        if (i != -1) {
          delete events[i];}} else {
        delete this.events[name];}}} else {
    delete this.events;
    this.events = {};}
  return this;};
...
Viewport.prototype.animate = function () {
  ...
  if (Math.abs(this.torqueX) > 1.0 || Math.abs(this.torqueY) > 1.0) {
    ...
    if (this.positionY > 360) {
      this.positionY -= 360;} else if (this.positionY < 0) {
      this.positionY += 360;}
    ...
    if (this.positionY > 90 && this.positionY < 270) {
      ...
      if (!this.upsideDown) {
        ...
        this.emit('upsideDown', { upsideDown: this.upsideDown });}} else {
      this.positionX += this.torqueX;
    ...
    if (this.positionX > 360) {
      this.positionX -= 360;} else if (this.positionX < 0) {
      this.positionX += 360;}}
...
Cube.prototype.rotateSides = function () {
  ...
  if (viewport.positionY > 90 && viewport.positionY < 270) {
    ...
    this.sides[5].getElementsByClassName('cube-image')[0].style[
      userPrefix.js + 'Transform'
    ] = 'rotate(' + -(viewport.positionX + 180 + viewport.torqueX) + 'deg)';} else {
    this.sides[0].getElementsByClassName('cube-image')[0].style[
      userPrefix.js + 'Transform'
    ] = 'rotate(' + (viewport.positionX - viewport.torqueX) + 'deg)';
    this.sides[5].getElementsByClassName('cube-image')[0].style[
